 ...
 * "Built for performance." The real Casselin logo settles on a luminous
 * industrial field under a slow push-in, a tricolore index line draws beneath
 * it, the positioning line resolves and the frame fades clean. The signature.
 */
export const Scene07Brand: React.FC = () => {
  const frame = useCurrentFrame();
  const { fps, durationInFrames } = useVideoConfig();

  const markScale = springScale(frame, fps, { delay: 6, from: 0.92, to: 1 });
  const push = eased(frame, [0, durationInFrames], [1.0, 1.05], EASE.soft);
  const lineW = eased(frame, [26, 50], [0, 520], EASE.expo);
  ...
  return (
    <AbsoluteFill style={{ opacity: fadeOut }}>
      <Background tint="cold" intensity={1.05} grid />
      <Stage>
        {/* central bloom */}
        <AbsoluteFill style={{ justifyContent: "center", alignItems: "center" }}>
          <div
            style={{
              ...
              height: 1100,
              borderRadius: "50%",
              background: `radial-gradient(circle, ${rgba(COLORS.cold, 0.24 * bloom)} 0%, transparent 62%)`,
        ...
        >
          <div
            style={{
              ...
              opacity: eased(frame, [6, 18], [0, 1], EASE.expo),
            }}
          >
            <Wordmark width={820} />
          </div>
          ...
          <div
            style={{
              ...
              boxShadow: `0 0 26px ${rgba(COLORS.cold, 0.6)}`,
            }}
          />
